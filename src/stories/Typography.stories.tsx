...
const headingCount: Extract<
  ITypographyProps["variant"],
  "h1" | "h2" | "h3" | "h4" | "h5" | "h6"
>[] = ["h1", "h2", "h3", "h4", "h5", "h6"];

const mockTypographyHeadings = (args: ITypographyProps) =>
  headingCount.map((heading, i) => (
    <Typography key={i} {...args} variant={heading}>
      Display {heading}
    </Typography>
  ));

const mockTypographyText = (args: ITypographyProps) => (
  <Typography {...args}>{args.children}</Typography>
);

const mockStorieVariants = ({
  storyVariant,
  ...args
...
const meta: Meta<ExtendedTypographyProps> = {
  ...
  decorators: [
    (_, { args }) =>
      StoryLayoutDecorator(mockStorieVariants(args), {
        className: "space-y-2",
      }),
  ],
  parameters: {
...
type TextVariantValueType = Extract<
  ITypographyProps["variant"],
  "xs" | "sm" | "md" | "lg" | "xl"
>;
...
type TextCustomWeightValueType = Extract<
  ITypographyProps["customWeight"],
  ITypographyProps["customWeight"]
>;
const CustomWeightValues: Record<
  keyof TextCustomWeightValueType,
  TextCustomWeightValueType
> = {
  bold: "bold",
  medium: "medium",
  regular: "regular",
